...
• 阶段 2：引入 AI（2 个月）：• 使用现有 LLM（如 GPT-4 或开源模型 Llama）通过 API 生成 Blender 脚本，参考 Blender Tutorial with ChatGPT.• 如果资源允许，fine-tune 小型模型（如基于 BlendNet 数据集 (Hugging Face BlendNet)).

• 阶段 3：反馈机制（1-2 个月）：• 软件错误反馈：检测脚本语法或运行时错误，优化生成（如 BlenderLLM 的错误率仅 3.4%）。• 结构力学反馈：集成 Blender 的有限元分析插件（如 BlenderFEA）或外部工具（如 PyNite）验证模型物理正确性。
